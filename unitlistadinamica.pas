...
type
  TDatoListaDin = TDatoConductores;
  TPunt = ^TNodo;
  TNodo = record
    Info: TDatoListaDin;
    Sig, Ant: TPunt;
...
procedure Eliminar(var L: TListaDin; Buscado: Cardinal; var x: TDatoListaDin);
...
begin
  New(Dir);
  Dir^.Info := x;
  if (L.Cab = NIL) or (LowerCase(L.Cab^.Info.ApYNom) > (LowerCase(x.ApYNom))) then
  begin
    Dir^.Sig := L.Cab;
    Dir^.Ant := NIL;
    if L.Cab <> NIL then
  ...
  begin
    Ant := L.Cab;
    L.Act := L.Cab^.Sig;
    while (L.Act <> NIL) and (LowerCase(L.Act^.Info.ApYNom) < LowerCase(x.ApYNom)) do
    begin
      Ant := L.Act;
      L.Act := L.Act^.Sig;
    end;
...
begin
  ...
  begin
    Ant := L.Cab;
    L.Act := L.Cab^.Sig;
    while (L.Act^.Info.DNI <> Buscado) do
    begin
      Ant := L.Act;
      L.Act := L.Act^.Sig;
    end;
